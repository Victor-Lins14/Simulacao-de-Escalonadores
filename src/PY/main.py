...
def round_robin(processos, quantum):
    lista = [
        {**p, "remaining": p["burst"], "color_idx": i}
    ...
    tme = sum(m["wt"]  for m in metricas) / len(metricas)
    tmt = sum(m["tat"] for m in metricas) / len(metricas)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Simulador de Escalonamento de CPU — Round Robin")
        self.configure(bg=BG)
        self.resizable(True, True)
        self.minsize(780, 500)

        self.process_rows = []   # lista de dicts {pid, arr_var, bst_var, frame}
        self.counter      = 0
    ...
    def _build_config(self, parent):
        ...
        cb = ttk.Combobox(left, textvariable=self.alg_var,
                          values=["Round Robin"], state="readonly",
                          font=("Segoe UI", 11))
        cb.pack(fill="x", pady=(4, 0), ipady=4)

        # Quantum
        right = tk.Frame(frm, bg=BG, width=200)
        right.pack(side="right", fill="y")
        right.pack_propagate(False)
        tk.Label(right, text="Quantum", bg=BG, fg=TEXT_MUTED,
                 font=("Segoe UI", 9, "bold")).pack(anchor="w")
        self.quantum_var = tk.StringVar(value="2")
        tk.Entry(right, textvariable=self.quantum_var,
    ...
    def _add_row(self, arrival="", burst=""):
        ...
        # PID
        tk.Label(row_frame, text=pid, bg=bg, fg=TEXT,
                 font=("Segoe UI", 11, "bold"),
                 width=12, anchor="center").pack(side="left", pady=8)
        ...
        btn = tk.Button(row_frame, text="🗑", bg=bg, fg=RED,
                        ...
                        command=lambda d=data: self._del_row(d))
    ...
    def _draw_gantt(self, gantt):
        ...
        for t in ticks:
            x = t * UNIT
            canvas.create_line(x, BAR_Y2, x, TICK_Y - 2,
                                fill=BORDER, width=1)
    ...
    def _draw_metrics(self, metricas, tme, tmt):
        ...
        for idx, m in enumerate(metricas):
            ...
            values = [m["pid"], m["arrival"], m["burst"],
                      m["ct"], m["tat"], m["wt"]]

            for i, val in enumerate(values):
                if i == 0:
                    # Badge colorido para o PID
                    badge_frame = tk.Frame(row, bg=bg_c)
                    badge_frame.pack(side="left", expand=True, fill="x",
                                     pady=8, padx=4)
                    tk.Label(badge_frame, text=str(val),
                             bg=gc_bg, fg=gc_fg,
                             font=("Segoe UI", 10, "bold"),
                             padx=10, pady=2,
                             relief="flat").pack()
                    ...
                    tk.Label(row, text=str(val), bg=bg_c, fg=TEXT,
                             font=("Segoe UI", 10),
                             anchor="center",
                             width=12).pack(side="left", expand=True,
                                            fill="x", pady=8)
        ...
        for label, val, desc in [
            ...
            c.pack(side="left", expand=True, fill="x",
                   padx=(0, 12) if label.startswith("TME") else 0, pady=4)
